...
ttyd(cmd="bash", port=1234, auth=st.secrets["ttyd_auth"], base_url='/r/localhost:1234')

tty_url = 'http://localhost:1234/r/localhost:1234'
...
st.text(
    f"server pid {peerTunnel.proc.pid if peerTunnel.is_alive() else 'dead'} "
